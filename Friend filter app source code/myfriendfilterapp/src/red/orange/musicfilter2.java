...
/**
 * Servlet implementation class musicfilter2
 */
@WebServlet("/musicfilter2")
public class musicfilter2 extends HttpServlet 
{
	...
	{	
		try{
			...
			String authURL = "https://graph.facebook.com/oauth/access_token?client_id=447748791951781&redirect_uri=http://localhost:8080/myfriendfilterapp/musicfilter2?parameterId="+parameterId+"&client_secret=35b28bee7543d230e7ec804382db97f9&code=" + code;		
			...
			ArrayList<String>  total_myfriendmusiclist_nowhitespace = new java.util.ArrayList<String>();
			...
			for(int i = 0; i <total_myfriendmusiclist.size() ; i++)// remove white space in names
				total_myfriendmusiclist_nowhitespace.add(total_myfriendmusiclist.get( i ).replaceAll("\\s",""));
			...
			/*for(String q :total_myfriendmusiclist)
				response.getWriter().println(q);*/
			response.getWriter().println("***********************************************");
			for(String q :total_myfriendmusiclist_nowhitespace)
				response.getWriter().println(q);
			
			// to find corresponding whitespace name
			int temp=-1;
			for(int j = 0; j < total_myfriendmusiclist_nowhitespace.size() ; j++)
			{
				if(parameterId.matches(total_myfriendmusiclist_nowhitespace.get(j)))
				{
					temp = j;
				}
			}
			...
			if(temp!=-1)
			{
				query2.setFullTextQuery(total_myfriendmusiclist.get(temp));// query Lady Gaga		
		...
		/////// to draw graph from google chart ////////
			...
			if (temp != -1)
			{
				...
				row.getCustomElements().setValueLocal("Music", total_myfriendmusiclist.get(temp));
			...
			if (temp == -1)
				...
				request.setAttribute("st",total_myfriendmusiclist.get(temp));
			
			request.setAttribute("total_myfriendmusiclist1",total_myfriendmusiclist);
			
			request.setAttribute("total_myfriendmusiclist_nowhitespace1", total_myfriendmusiclist_nowhitespace);
			...
			RequestDispatcher rd = request.getRequestDispatcher("showfriendlist2.jsp");
